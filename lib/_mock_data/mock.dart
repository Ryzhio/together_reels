import 'package:together_reels/models/reel.dart';
import 'package:together_reels/models/user.dart';

User currentUser =
    User('id', 'stackedlist', 'https://picsum.photos/id/1062/400/400');

User demoUser1 = User('id1', 'Arrow205',
    'https://cdn.pixabay.com/photo/2015/01/08/18/25/desk-593327_960_720.jpg');

User demoUser2 = User('id2', 'Willtools_king',
    'https://cdn.pixabay.com/photo/2012/12/24/08/39/animal-72186_960_720.jpg');

User demoUser3 = User('id3', 'King_true',
    'https://cdn.pixabay.com/photo/2016/11/06/05/36/lake-1802337_960_720.jpg');

User demoUser4 = User('id4', 'Lillit445',
    'https://cdn.pixabay.com/photo/2014/10/07/13/48/mountain-477832_960_720.jpg');

User demoUser5 = User('id5', 'Desconocido1998',
    'https://cdn.pixabay.com/photo/2021/03/10/02/18/anime-6083347_960_720.jpg');

final List<Reel> reels = [
  Reel(
      demoUser4,
      'https://cdn.pixabay.com/photo/2012/12/24/08/39/animal-72186_960_720.jpg',
      'Maluma - Hawai',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '3.8k',
      '60',
      false,
      true),
  Reel(
      demoUser3,
      'https://cdn.pixabay.com/photo/2015/01/08/18/25/desk-593327_960_720.jpg',
      'Karol G - Bichota',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '3.5k',
      '30',
      false,
      true),
  Reel(
      demoUser2,
      'https://cdn.pixabay.com/photo/2016/11/06/05/36/lake-1802337_960_720.jpg',
      'This Love - Bob Marley',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '2.4k',
      '20',
      false,
      true),
  Reel(
      demoUser4,
      'https://cdn.pixabay.com/photo/2014/10/07/13/48/mountain-477832_960_720.jpg',
      'Soda Stereo - Persiana Americana',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '5.1k',
      '40',
      false,
      true),
  Reel(
      demoUser2,
      'https://cdn.pixabay.com/photo/2021/03/10/02/18/anime-6083347_960_720.jpg',
      'Electro House - Electronica',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '3.9k',
      '45',
      false,
      true),
  Reel(
      demoUser3,
      'https://cdn.pixabay.com/photo/2015/01/08/18/25/desk-593327_960_720.jpg',
      'Alejandro saenz - Corazón Partio',
      'muy genial!!!! Todo en esta vida se puede lograr. Esfuerzate',
      '1.5k',
      '10',
      false,
      true)
];
